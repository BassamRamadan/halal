...
class CashedData: NSObject  {
    ...
    class func saveCartData (data:SacrificesDetails?){
        let category = SacrificesDetails(id: data?.id ?? 0, name: data?.name, description: data?.description, imagePath: nil, price: data?.price, mincementPrice: data?.mincementPrice, shin: data?.shin ?? false, head: data?.head ?? false, bowels: data?.bowels ?? false, cocking: data?.cocking, packages: data?.packages, sizes: data?.sizes, slices: data?.slices)
        let encodedData = NSKeyedArchiver.archivedData(withRootObject: category)
        let userDefaults = UserDefaults.standard
        userDefaults.set(encodedData, forKey:  "CartData")
        userDefaults.synchronize()
    }
    class  func getCartData ()->SacrificesDetails? {
        let dif = UserDefaults.standard
        return (dif.object(forKey : "CartData") as? SacrificesDetails)
    }
    // for admin
